...
def tflite_detect_images(image_path, save=True):
    ...
    for i in range(len(scores)):
        if ((scores[i] > 0.2) and (scores[i] <= 1.0)):
            ...
            #print(True)
            if save:
                os.makedirs("/var/openflexure/data/micrographs/detections", exist_ok=True)
                cv2.rectangle(image, (xmin,ymin), (xmax,ymax), (10, 255, 0), thickness = 3)
                cv2.imwrite(f'/var/openflexure/data/micrographs/detections/{os.path.basename(image_path)}', image)
                logging.info(f'File with detections saved to .../detections/{os.path.basename(image_path)}')
            return True
    #print(False)
    return False
def write_detection_to_txt(folder_path, save=True):
    found_detections = False
    with open(os.path.join(folder_path, 'detections.txt'), 'w') as w:
        for image in os.listdir(folder_path):
            detection = tflite_detect_images(os.path.join(folder_path, image), save)
            if detection:
...
## define an array consisting of arrays of type ['Alloy', position]
## example ['AA7085', (0,0,0)]
## or a full array for a two sample scan
## [['AA7085', (0,0,0)],['AA7449', (3200, 1600, 0)]] etc
scan_array = [["AA7085", (0, 0, 0)], ["AA7049", (1600, 1600 , 0)], ["AA7885", (3200, 3200, 0)]]
n_of_scans = 10 #decide on number of scans here
detect =False #detect parameter decides if we run crack detection on the captured images
save = False #save parameter decides if we save the images of detected cracks with bounded boxes
class TileScanAPI(ActionView):
    ...
    def post(self, args):
        ...
        with microscope.lock(timeout=1):
            for scan_n in range(n_of_scans):
                for sample in scan_array:
                    microscope.stage.move_abs(sample[1])
                    # Run scan_extension_v2
                    self.extension.tile(
                        microscope,
                        basename=f"{sample[0]}_{scan_n}",
                    ...
                    if detect:
                        path = "/var/openflexure/data/micrographs/"
                        directory = f"SCAN_{sample[0]}_{scan_n}"
                        write_detection_to_txt(os.path.join(path, directory), save=save)
